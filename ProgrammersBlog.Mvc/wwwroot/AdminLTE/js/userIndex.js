...
/* Ajax GET / Getting the _UserUpdatePartial as Modal Form starts from here. */
    ...
    $(function () {
        ...
        /* Ajax POST / Updating a RolAssing starts from here */

        placeHolderDiv.on('click',
            '#btnAssing',
            function (event) {
                event.preventDefault();
                const form = $('#form-role-assing');
                const actionUrl = form.attr('action');
                const dataToSend = new FormData(form.get(0));
                $.ajax({
                    url: actionUrl,
                    ...
                    contentType: false,
                    success: function (data) {
                        const userRoleAssingAjaxModel = jQuery.parseJSON(data);
                        console.log(userRoleAssingAjaxModel);
                        const newFormBody = $('.modal-body', userRoleAssingAjaxModel.RoleAssingPartial);
                        placeHolderDiv.find('.modal-body').replaceWith(newFormBody);
                        const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                        if (isValid) {
                            const id = userRoleAssingAjaxModel.UserDto.User.Id;
                            const tableRow = $(`[name="${id}"]`);
                            placeHolderDiv.find('.modal').modal('hide');
                            toastr.success(`${userRoleAssingAjaxModel.UserDto.Message}`, "Başarılı İşlem!");
                        } else {
                            let summaryText = "";
                            $('#validation-summary > ul > li').each(function () {
                                let text = $(this).text();
